...
(function (factory) {
	...
	*/
	...
	function formatRange(date1, date2, formatStr, separator, isRTL, isJalaali) {
		...
		separator = (isJalaali) ? ' الی ' : separator || ' - ';
	...
	*/
	...
	function Calendar_constructor(element, overrides) {
		...
		function prev() {
			...
			//تاریخ کنونی تقویم
			var viewDate = currentView.calendar.getDate()._d.toISOString().split('T')[0];

			date = currentView.computePrevDate(date);

			//اگر تقویم در حالت ماه بود
			if (currentView.name == "month") {
				//به دست آوردن تاریخ کنونی به شمسی
				var currentPersianDate = dpx.lib.date.toJalaaliDate(viewDate);
				//حل مشکل تبدیل به تاریخ شمسی در هنگام زدن دکمه قبلی
				var lastMonthPersianDate = currentPersianDate.split('/')[0] + '/' + ('0' + (currentPersianDate.split('/')[1] - 1)).slice(-2) + '/' + '01';
				date._d = new Date(dpx.lib.date.toGregorianDate(lastMonthPersianDate));
			}
	...
	*/
	...
	Calendar.defaults = {
		...
		//	debugger
		//},
		//eventAfterAllRender: function (event, $el, view) {
		//	debugger
		//}
	};
